				   ...
				   char *buf)
{
	...
	if (((pu_reason & BIT(PU_REASON_EVENT_HWRST)) &&
	     qpnp_pon_power_off_reason() == PON_POWER_OFF_REASON_PS_HOLD) ||
	...
	if (qpnp_pon_power_off_reason() == PON_POWER_OFF_REASON_KPDPWR_N &&
...
static int cpumaxfreq_show(struct seq_file *m, void *v)
{
	...
	case QCOM_HW_PLATFORM_GRUS:
		seq_printf(m, "2.3\n");
		break;
	case QCOM_HW_PLATFORM_PYXIS:
	case QCOM_HW_PLATFORM_SIRIUS:
	case QCOM_HW_PLATFORM_VELA:
		seq_printf(m, "2.2\n");
		break;
	default:
		seq_printf(m, "2.2\n");
